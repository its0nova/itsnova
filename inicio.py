...
def agrega_percepciones():
    if request.method == 'POST':
        ...
        aux_diaper = request.form['autorizar']
        ...
        #else:
        cursor.execute('insert into percepcion (Descripcion, id_periodo, Dia, Mes, Nom_per, Tipo_nom,  Valor, Dia_per) values (%s,%s,%s,%s,%s,%s,%s,%s)'
        ,(aux_conc, aux_per,aux_dia, aux_mes, aux_t_con, aux_t_nom,aux_valor,aux_diaper))
        conn.commit()
        
        cursor.execute('select id_percepcion, Descripcion, id_periodo, Dia, Mes, Nom_per, Tipo_nom, Valor, Dia_per '
                   'from percepcion where id_percepcion = (select max(id_percepcion) from percepcion )' )
        datos=cursor.fetchall()

        cursor.execute('select id_periodo, Descripcion from periodo')
        datos1 = cursor.fetchall()
        cursor.execute('select Dia, Descripcion from percepcion')
        datos3 = cursor.fetchall()
        cursor.execute('select Mes, Descripcion from percepcion')
        datos4 = cursor.fetchall()
        cursor.execute('select Nom_per, Descripcion from percepcion')
        datos5 = cursor.fetchall()
        cursor.execute('select Tipo_nom, Descripcion from percepcion')
        datos2 = cursor.fetchall()
        cursor.execute('select Valor, Descripcion from percepcion')
        datos6 = cursor.fetchall()
        cursor.execute('select Dia_per, Descripcion from percepcion')
        datos7 = cursor.fetchall()
        cursor.execute('select idtipo_nomina, descripcion from tipo_nomina')
        datos8 = cursor.fetchall()
        cursor.execute('select idPuesto, Descripcion from puesto')
        datos9 = cursor.fetchall()
        cursor.execute('select * from per_puesto where id_percepcion =%s',(id))
        datos10 = cursor.fetchall()
        
        

    return render_template("edi_percepcion.html",percepciones=datos, periodos=datos1, percepcion=datos2, dia=datos3, mes=datos4, tipo_concepto=datos5, valor=datos6, tipo=datos7, tipos=datos8, puestos=datos9, perPuesto=datos10 )

        #conn.close()
        #return redirect(url_for('percepciones'))
        
@app.route('/ed_percepcion/<string:id>')
def ed_percepcion(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_percepcion, Descripcion, id_periodo, Dia, Mes, Nom_per, Tipo_nom, Valor, Dia_per '
                   'from percepcion where id_percepcion =%s',(id))
    datos=cursor.fetchall()
    cursor.execute('select id_periodo, Descripcion from periodo')
    datos1 = cursor.fetchall()
    cursor.execute('select Dia, Descripcion from percepcion')
    ...
    cursor.execute('select idPuesto, Descripcion from puesto')
    datos9 = cursor.fetchall()
    cursor.execute('select * fromdatos9 = cursor.fetchall()')
    cursor.execute('select * from per_puesto where id_percepcion =%s',(id) )
    datos10 = cursor.fetchall()
...
@app.route('/ag_puesP/<string:id>',methods=['POST'])
def ag_puesP(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    ...
    if request.method == 'POST':
        ...
        aux_montoPuesto = request.form['montoPuesto']
        aux_periodoPuesto = request.form['periodoPuesto']

        cursor.execute('insert into per_puesto (id_percepcion, id_puesto, monto, id_periodo) values (%s,%s,%s,%s)', (id, aux_Puesto, aux_montoPuesto, aux_periodoPuesto))
        conn.commit()
...
@app.route('/del_puesP/<string:id>/<string:idP>')
def del_puesP(id,idP):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()



    cursor.execute('delete from  per_puesto  where idperpuesto = %s and id_percepcion =%s',(idP,id))
    conn.commit()
...
def modifica_percepciones(id):
    if request.method == 'POST':
        ...
        cursor.execute('update percepcion set Descripcion=%s, id_periodo=%s, Dia=%s, '
                       'Mes=%s, Nom_Per=%s, Tipo_nom=%s, Valor=%s, Dia_per=%s '
                       'where id_percepcion= %s',(aux_conc,aux_per,aux_dia,aux_mes,aux_t_con,aux_t_nom,aux_valor,aux_diaper,id))
        conn.commit()
        conn.close()
    return redirect(url_for('percepciones'))

@app.route('/bo_percepcion/<string:id>')
def bo_percepcion(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
...
def isr():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_isr, limite_inf, limite_sup, cuota_fija, porcentaje  from tabla_isr order by id_isr')
    datos=cursor.fetchall()
    conn.close()
    return render_template("tabla_isr.html", isrs=datos)

@app.route('/agr_isr')
def agr_isr():
    return render_template("agr_isr.html")
    
@app.route('/isr_agrega', methods=['POST'])
def isr_agrega():
    if request.method == 'POST':
        aux_lim_inf = request.form['limite_inferior']
        li=float(aux_lim_inf)
        aux_lim_sup = request.form['limite_superior']
        ls=float(aux_lim_sup)
        aux_cuota = request.form['cuota_fija']
        aux_porcentaje = request.form['porcentaje']
        p=float(aux_porcentaje)
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        if (li>ls or p>100):
            cursor.execute('Select count(*) from tabla_isr WHERE (limite_inf >= %s AND limite_inf <= %s) OR (limite_sup >= %s AND limite_sup <= %s)', (aux_lim_inf, aux_lim_inf, aux_lim_sup, aux_lim_sup))
            incorrecto = cursor.fetchone()
            print(incorrecto)
            if (incorrecto[0] != 0):
                error = "Este rango ya se encuentra agregado."
                return render_template("error.html", des_error=error, paginaant="/cal_isr")
            else:
                error = "El límite inferior no puede ser mayor que el límite superior y/o el porcentaje es mayor a 100'%'. "
                return render_template("error.html", des_error=error, paginaant="/cal_isr")                
        else:
            cursor.execute('insert into tabla_isr (limite_inf, limite_sup, cuota_fija, porcentaje) values (%s,%s,%s,%s)', (aux_lim_inf, aux_lim_sup, aux_cuota, aux_porcentaje))
            conn.commit()
            conn.close()  
    return  redirect(url_for('isr'))
...
def ed_isr(id):
    ...
    dato=cursor.fetchall()
    conn.close()
    return render_template("edi_isr.html", isr = dato[0])

@app.route('/modi_isr/<string:id>', methods=['POST'])
def modi_isr(id):
    if request.method == 'POST':
        aux_lim_sup = request.form['limite_superior']
        ls=float(aux_lim_sup)
        ...
        if (li>ls or p>100):
            if li>ls:
                error = " EL LÍMITE INFERIOR ES MAYOR AL LÍMITE SUPERIOR."
                return render_template("error.html", des_error=error, paginaant="/isr")
            else:
                error = " EL PORCENTAJE ES MAYOR A 100."
                return render_template("error.html", des_error=error, paginaant="/isr")
        else:                    
            conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
            cursor = conn.cursor()
            cursor.execute('update tabla_isr set limite_inf=%s, limite_sup=%s, cuota_fija=%s, porcentaje=%s where id_isr=%s', ( aux_lim_inf, aux_lim_sup, aux_cuota, aux_porcentaje,id))
            conn.commit()
            conn.close()
...
def base_cot():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_base, descripcion, calculo  from base_imss order by id_base')
    datos=cursor.fetchall()
    conn.close()
    return render_template("tabla_base_cot.html", obreras=datos)

@app.route('/agr_bas_cot')
def agr_bas_cot():
    return render_template("agr_bas_cot.html")
    
@app.route('/agrega_bas_cot', methods=['POST'])
def agrega_bas_cot():
    if request.method == 'POST':
        aux_desc = request.form['descripcion']
        aux_calc = request.form['calculo']
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('select count(*) from base_imss where descripcion = %s',(aux_desc))
        cursor.execute('insert into base_imss (descripcion,  calculo)'
                        'values (%s,%s)',
                        (aux_desc, aux_calc))
        conn.commit()
        conn.close()
        return redirect(url_for('base_cot'))

@app.route('/bo_base_cot/<string:id>')
def bo_base_cot(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
...
@app.route('/ed_base_cot/<string:id>')
def ed_base_cot(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    ...
    dato=cursor.fetchone()
    conn.close()
    return render_template("edi_base_cot.html", obreras=dato)

@app.route('/modi_base_cot/<string:id>', methods=['POST'])
def modi_base_cot(id):
    if request.method == 'POST':
        aux_desc = request.form['descripcion']
        aux_calc = request.form['calculo']
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('update base_imss set descripcion=%s, calculo=%s where id_base=%s', ( aux_desc,aux_calc,id))
        conn.commit()
        conn.close()
    return redirect(url_for('base_cot'))
...
def imss():
    conn = pymysql.connect(host='localhost', user='root',passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select  id_cpto, descripcion, porcentaje, id_base from tabla_imss order by descripcion')
    datos = cursor.fetchall()
    conn.close()
    return render_template("tabla_imss.html", imss=datos)

@app.route('/agr_imss')
def agr_imss():
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_base, descripcion from base_imss')
    datos = cursor.fetchall()
    conn.close()
    return render_template("agrega_imss.html", obreras=datos)

@app.route('/agrega_imss', methods=['POST'])
def agrega_imss():
    if request.method == 'POST':
        aux_desc = request.form['descripcion']
        aux_porcentaje = request.form['porcentaje']
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('select count(*) from tabla_imss where Descripcion = %s', (aux_desc))
        ...
        if (datos1[0] != 0):
            ...
            cursor.execute('insert into tabla_imss (descripcion, porcentaje, id_base) values (%s, %s, %s)',(aux_desc, aux_porcentaje, aux_base))
            conn.commit()
            cursor.execute('select descripcion from base_imss order by Descripcion')
            datos = cursor.fetchall()
            conn.close()
        return redirect(url_for('imss'))

@app.route('/ed_imss/<string:id>')
def ed_imss(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('select id_cpto, descripcion, porcentaje, id_base from tabla_imss where id_cpto = %s', (id))
    dato=cursor.fetchone()
    
    cursor.execute('select id_base, descripcion from base_imss')
    datos = cursor.fetchall()
    conn.close()
    return render_template("edi_imss.html", imss=dato, obreras=datos)
        
@app.route('/modi_imss/<string:id>', methods=['POST'])
def modi_imss(id):
    if request.method == 'POST':
        aux_desc = request.form['descripcion']
        aux_porcentaje = request.form['porcentaje']
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
        cursor = conn.cursor()
        cursor.execute('update tabla_imss set descripcion=%s, porcentaje=%s, id_base=%s where id_cpto=%s', ( aux_desc,aux_porcentaje,aux_base,id))
        conn.commit()
        cursor.execute('select descripcion from base_imss order by Descripcion')
        conn.close()
    return redirect(url_for('imss'))

@app.route('/bo_imss/<string:id>')
def bo_imss(id):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='r_humanos')
    cursor = conn.cursor()
    cursor.execute('delete from tabla_imss where id_cpto = {0}'.format(id))
    conn.commit()
    conn.close()
    return redirect(url_for('imss'))
